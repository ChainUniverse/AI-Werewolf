...
import type { PlayerServer } from '../../lib/PlayerServer';

function formatPlayerList(alivePlayers: Array<{id: number; isAlive: boolean}>): string {
  return alivePlayers.filter(p => p.isAlive).map(p => p.id).join(', ');
}

function formatSpeechHistory(speeches: Array<{playerId: number; content: string; type?: string}>): string {
  return speeches.map(s => `${s.playerId}号: ${s.content}`).join('，') || '暂无发言记录';
}
...
// 工厂函数
export function getRoleSpeech(playerServer: PlayerServer, context: PlayerContext | SeerContext | WitchContext): string {
